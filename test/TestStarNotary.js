...
it('lets 2 users exchange stars', async() => {
    ...
    //Testing that exchange fails if neither participant is the sender
    await truffleAssert.reverts(instance.exchangeStars(7, 8, {from: invalidUser}), "Exchange must be initiated by an owner of one of the stars");
    assert.equal(await instance.ownerOf.call(tokenId1), user1);
    assert.equal(await instance.ownerOf.call(tokenId2), user2);
    //Testing successful exchange
...
it('lets a user transfer a star', async() => {
    ...
    //Testing that transfer fails if sender is not owner
    await truffleAssert.reverts(instance.transferStar.call(user2, tokenId, {from: user2}), "Transfer must be initiated by star owner");
    assert.equal(await instance.ownerOf.call(tokenId), user1);
